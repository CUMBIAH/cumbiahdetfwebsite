...
Each CUMBIAH lander carries a small cluster of four hydrophones, and that cluster measures the 3D bearing to any sound it detects. If two or more landers detect the same porpoise click, the bearings can be triangulated and the porpoise located. None of that works, though, unless we know two things about every lander to within a metre or two and a degree or two:

1.  **Where it is.** A lander is dropped over the side and falls 17 m through a tidal stream. It does not land where the boat's GPS said it went in -- in deployment 1 the landers ended up between 4 m and 20 m from their drop positions.
2.  **Which way it is pointing.** The lander lands on whatever the seabed gives it, so its heading, pitch and roll are unknown. A bearing measured by the cluster is relative to the cluster, and until the orientation is known it cannot be turned into a bearing relative to north.

To solve both, the boat ran a set of calibration lines over the landers straight after each deployment. The propeller cavitation is a loud, broadband transient that every lander detects thousands of times, and the boat's own GPS tells us where each of those transients was made. That gives us a large set of (bearing measured at the lander, known source position) pairs, and from those the position and orientation of the lander can be recovered with a grid search.
...
The calculation itself lives in `locateplatform.m`. It draws nothing -- everything the plots need comes back in a `results` struct -- so `locate_platform.m` is the script you actually run, and `locate_array.m` is the same calculation in a loop.
...
If you have not met a grid search before, it is worth a paragraph, because the rest of this page is really just three grid searches in a row.

There is no formula you can feed the bearings into that hands back the lander's position. With six unknowns, thousands of noisy measurements and angles that wrap around at ±180°, working backwards algebraically is a nightmare. So we do not. Instead we work *forwards*, thousands of times over, and pick the answer that fits best.

The recipe is:

1.  **Propose an answer.** Lay an imaginary grid over the seabed -- say every metre across an 80 × 80 m square centred on the drop position -- and take one grid point as a candidate: *suppose the lander is right here*.
...
It is brute force, and unashamedly so. The appeal is that it needs nothing clever: you only need to be able to predict the measurement for a given answer, which is straightforward geometry, and the computer can afford to try tens of thousands of answers.

::: {.callout-tip}
## The surface is worth more than the answer

Because every grid point gets a score, the output is not a single position -- it is a whole **surface** of scores, and that surface is where most of the diagnostic value lives. Plot it (as in @fig-coarse-grid and @fig-fine-grid) and you can see at a glance how trustworthy the answer is:
...
A biologist's analogy: a grid search is the acoustic equivalent of gridding a study site into quadrats and sampling every one, rather than trying to deduce where the animals are from first principles. You end up with a map of how good each cell is, and the shape of that map tells you whether your "best" cell is genuinely the best or just the luckiest of many similar ones.
:::
...
Propeller cavitation produces an enormous number of detections, and as noted above the work scales with grid points × clicks. `locateplatform` therefore keeps at most `maxbearings` bearings per second (default 5), spread evenly through each second, which is plenty to constrain six parameters and keeps the search to minutes rather than hours.
:::
...
The calibration lines were always run immediately after the landers went down, so the transients are in the first hour or so of each deployment. Open PAMGuard in **viewer mode** on the lander's database and binary store, go to the ***Data Map*** tab and navigate to the start of the deployment. The calibration shows up as a dense, continuous block of click detections quite unlike anything else in the dataset -- the boat is essentially shouting at the lander for half an hour.

Right click on that block and select ***Centre Data Here*** to load it.

![**Placeholder.** The data map at the start of the deployment. The calibration lines appear as a solid block of detections in the first hour, much denser than any porpoise encounter.](../images/locatelanders/placeholder_datamap.png){#fig-mark-datamap}
...
![**Placeholder.** Cavitation transients in the bearing--time display. The bearing sweeps smoothly through the pass, and the slant angle spikes as the boat goes directly overhead.](../images/locatelanders/placeholder_transients.png){#fig-mark-transients}

## Marking them up

Use the area selection tool exactly as for a porpoise click train: double click on the bearing--time display, draw a box around the transients, and join the ends of the dotted line so the selected area goes grey. Then right click inside the grey area and select ***Label Clicks*** (or press *Ctrl+L*).

![**Placeholder.** Marking out a run of cavitation transients with the area selector.](../images/locatelanders/placeholder_areaselect.png){#fig-mark-area}
...
![**Placeholder.** Labelling the marked clicks as a `bc` event.](../images/locatelanders/placeholder_bcevent.png){#fig-mark-event}
...
-   **Do not clean up the way you would for a porpoise.** The search is a least-squares fit over thousands of bearings, so a scattering of echoes and bad time delays is averaged out. Removing them by hand is not worth the effort.
-   **Several events are fine.** `locateplatform` pools every `bc` event in the day folder, so one event per calibration line is the natural way to work.
:::

Once the events are saved, the annotated database (`lander<N>_Denmark_database_annotated.sqlite3`) holds the calibration events and the binary store holds the clicks themselves. That is everything the MATLAB side needs.
...
|--------------------|----------------------------------------------------|
| [`cumbiah_landers_mat`](https://github.com/CUMBIAH/cumbiah_landers_mat) | The library itself: `locateplatform`, `getlanderdeploymentinfo`, `getlanderdatapaths`, `getlandersuperfolder` |
...
| SMRU `soundnetlocalisation` utilities | `grdsrch_pos`, `grdsrch_pos_hpr`, `st_loc_chi2`, `interpsrcloc`, `plot_chi2_surf`, `plot_clk_src_bearings` -- the grid search and its plotting helpers |
| SMRU shared utilities | `latLong2meters`, `latLong2bearing`, `getsuperfolder`, `getdefaultcols` |
...
| The Denmark data folder | Resolved by `getlandersuperfolder`, which sits on `getsuperfolder`. `getsuperfolder` maps the machine's hostname to the Dropbox root, so a new machine needs a case adding to it |

::: {.callout-warning}
## If MATLAB cannot find the data

`getlanderdatapaths` does not check that any path exists -- a lander that has not been processed yet returns the path its data *will* have. If the localisation fails with a missing-file error, check `data.sqlitedB`, `data.binaryfolder` and `data.gpsfile` against what is actually on disk before looking anywhere else.
:::

## Running it

Open `locate_landers/locate_platform.m`, set the lander and the deployment at the top, and run it:

``` matlab
%which lander (1-9) and which deployment
...
`getlanderdeploymentinfo` turns the lander and deployment numbers into a start time and a SoundTrap serial number, and `getlanderdatapaths` turns those into the database, the binary store, the boat GPS track and the drop position. Not every lander was in every deployment -- only landers 1, 5, 7 and 8 were redeployed in Feb 25, and lander 8 never released in June 25 -- and asking for a combination that never happened raises an error listing the deployments that lander *does* have.
